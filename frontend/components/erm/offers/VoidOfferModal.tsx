...
export default function VoidOfferModal({ offer, open, onClose, onApplied }: Props) {
    ...
    void (async () => {
      try {
        const res = await api.get<ReasonGroup[]>(
          '/api/v1/erm/offers/reason-codes?family=VOID',
        );
        setGroups(res.data ?? []);
      } catch {
        setGroups([]);
  ...
  async function submit() {
    ...
    setSubmitting(true);
    try {
      await api.post(`/api/v1/erm/offers/${offer.id}/void`, {
        reasonCode,
      ...
      setErr(
        ax.response?.data?.error ??
          (e instanceof Error ? e.message : 'Void failed'),
      );
    } finally {
      setSubmitting(false);
    }
